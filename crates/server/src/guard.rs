...
/// The pages serve their own CSS, fonts and one small script from the
/// same origin, and fetch only from it, so the policy can say exactly that.
const CSP: &str = "default-src 'none'; style-src 'self'; font-src 'self'; img-src 'self' data:; \
...
pub fn is_operator_path(method: &Method, path: &str) -> bool {
    ...
    match parts.as_slice() {
        ...
        ["api", "invitations", "unclaimed"] => get,
        ["api", "invitations", "purge"] => post,
        ["api", "reports"] => get,
        ["api", "forge", "metrics"] => get,
...
mod tests {
    ...
    fn the_operators_door_is_a_known_list() {
        for (method, path) in [
            ...
            (Method::GET, "/api/invitations/unclaimed"),
            (Method::POST, "/api/invitations/purge"),
            (Method::GET, "/api/reports"),
            (Method::GET, "/api/forge/metrics"),
