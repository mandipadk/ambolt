...
async fn a_rename_moves_everything_and_the_old_name_is_gone() {
    ...
    let app = &forge.app;
    let id = a_change_in(&forge, "ada/demo").await;
    ...
    assert!(repos.join("ada/shown.git").is_dir() && !repos.join("ada/demo.git").exists());
    // The old address is not gone: it points at the new one, for anyone
    // who may read it there. To a stranger, or to somebody who cannot
    // read the repository, the old name is as empty as any other.
    let (status, _) = api(app, "GET", "/api/repos/ada/demo", "ada", None).await;
    assert_eq!(status, StatusCode::PERMANENT_REDIRECT);
    let (status, _) = api_anonymous(app, "GET", "/api/repos/ada/demo/changes", None).await;
    assert_eq!(
        status,
        StatusCode::NOT_FOUND,
        "a stranger learns nothing from an old name"
    );
    api(
        app,
        "POST",
    ...
    .await;
    let (status, _) = api(app, "GET", "/api/repos/ada/demo", "bee", None).await;
    assert_eq!(
        status,
        StatusCode::NOT_FOUND,
        "nor does somebody who cannot read it"
    );
    let (status, location) = get_redirect(app, "/ada/demo/changes", "").await;
    assert_eq!(
        status,
        StatusCode::SEE_OTHER,
        "a stranger on a page is sent to sign in"
    );
    assert!(location.starts_with("/login"), "{location}");
    // Once the repository is public, a stranger is sent where it went.
    api(
        app,
        "POST",
        "/api/repos/ada/shown/visibility",
        "ada",
    ...
    let (_, changes) = api(app, "GET", "/api/repos/ada/shown/changes", "ada", None).await;
    ...
    git(
        ...
        &[
            "clone",
            &format!(
                "http://ada:{}@{}/git/ada/shown",
                forge.ada_token, forge.addr
            ),
            "wc",
        ],
    );
...
async fn an_archived_repository_takes_nothing_new_until_it_is_unarchived() {
    let forge = boot().await;
    let app = &forge.app;
    let (status, _) = api(app, "POST", "/api/repos/ada/demo/archive", "ada", None).await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, refused) = api_with_token(
        ...
        "/api/changes",
        &forge.scout_token,
        Some(json!({ "repo": "ada/demo", "target": "main", "title": "Too late" })),
    )
    .await;
    ...
    let (status, _) = api(
        ...
        "/api/tasks",
        "ada",
        Some(json!({ "title": "t", "spec": "s", "repo": "ada/demo" })),
    )
    .await;
    assert_eq!(status, StatusCode::CONFLICT);
    // Reading goes on.
    let (status, _) = api(app, "GET", "/api/repos/ada/demo/changes", "ada", None).await;
    assert_eq!(status, StatusCode::OK);
    // Twice is refused; unarchiving reopens it.
    let (status, _) = api(app, "POST", "/api/repos/ada/demo/archive", "ada", None).await;
    assert_eq!(status, StatusCode::CONFLICT);
    let (status, _) = api(app, "POST", "/api/repos/ada/demo/unarchive", "ada", None).await;
    assert_eq!(status, StatusCode::OK);
    a_change_in(&forge, "ada/demo").await;
    // From the settings page, the same.
    let (_, cookie) = sign_in_as(&forge, "ada").await;
    let (status, location) =
        post_form(app, "/ada/demo/settings/archive", &cookie, "archived=yes").await;
    assert_eq!(status, StatusCode::SEE_OTHER);
    assert_eq!(location, "/ada/demo/settings?done=1");
    let (_, settings) = page_with_cookie(app, "/ada/demo/settings", &cookie).await;
...
async fn deleting_needs_the_name_typed_and_takes_the_repository_away() {
    ...
    assert_eq!(location, "/");
    assert!(
        !repos.join("ada/demo.git").exists(),
    ...
    assert_eq!(status, StatusCode::NOT_FOUND);
    let (status, _) = get_redirect(app, "/ada/demo", &cookie).await;
    assert_eq!(status, StatusCode::NOT_FOUND);
